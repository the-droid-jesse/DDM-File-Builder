...
async function parseXLSX(arrayBuffer) {
  ...
  // Find the first row with actual data (headers)
  let headerRowIdx = 0;
  ...
  // Fill empty header names to avoid duplicates like "" overwriting each other
...
Return ONLY valid JSON, no markdown:
{
  ...
  const runMapping = () => {
    if (!ddmFile) {
      addMsg("agent", "⚠️ Load the DDM file first.");
      return;
    }
          ...
          {uuidFile && ddmFile && (
            <div style={{ background: "#070f1d", border: "1px solid #0f2744", borderRadius: 10, padding: 11 }}>
                ...
                <div style={{ display: "flex", gap: 4 }}>
                  {["phone", "email"].map(t => (
                    <button key={t} onClick={() => { setMatchType(t); setRefIdCol(""); setDdmIdCol(""); }}
                      style={{ flex: 1, padding: "5px 0", borderRadius: 5, fontSize: 10, fontWeight: 700, fontFamily: "inherit", cursor: "pointer", textTransform: "uppercase",
                  ...
                  {AGDC_OUTPUT_FIELDS.filter(f => f.num >= 2 && f.num <= 10).map(f => (
                    <div key={f.key} style={{ display: "flex", gap: 6, alignItems: "flex-end" }}>
                      ...
                      {f.num === 9 && (
                        ...
                        }} style={{ marginBottom: 8, height: 26, background: fallbackState ? "#16a34a" : "#1e3a5f", color: "#fff", border: "none", borderRadius: 5, padding: "0 10px", fontSize: 10, cursor: "pointer", fontFamily: "inherit" }}>
                          {fallbackState ? `Fallback: ${fallbackState}` : "➕ Auto F9 State"}
                        </button>
                      )}
                    </div>
                  ))}
              ...
              <button onClick={runMapping} disabled={!ddmFile}
                style={{ width: "100%", background: ddmFile ? "linear-gradient(135deg,#0ea5e9,#6366f1)" : "#1e293b", color: "#fff", border: "none", borderRadius: 5, padding: "8px 0", fontSize: 12, fontWeight: 700, cursor: ddmFile ? "pointer" : "not-allowed", fontFamily: "inherit" }}>
                ▶ Run Mapping
              </button>
            </div>
          )}
          ...
          <div ref={chatRef} style={{ flex: 1, overflowY: "auto", padding: 11, display: "flex", flexDirection: "column", gap: 7 }}>
            ...
            {messages.map(m => (
              <div key={m.ts} style={{ display: "flex", justifyContent: m.role === "user" ? "flex-end" : "flex-start" }}>
                <div style={{ maxWidth: "88%", borderRadius: 8, padding: "7px 10px", fontSize: 12, lineHeight: 1.6, whiteSpace: "pre-wrap",
              ...
              <thead>
                <tr>{result.headers.map((h, i) => (
                  <th key={h+i} style={{
                    ...
                    color: h === "Loyalty ID/Rewards Number" ? "#22c55e" : h === "Management Account Number / Retail Control Number" ? "#f59e0b" : "#38bdf8",
                    background: h === "Loyalty ID/Rewards Number" ? "#0c2a1a" : h === "Management Account Number / Retail Control Number" ? "#1a1200" : "transparent"
                  }}>{h}</th>
                ))}</tr>
              </thead>
              <tbody>
                  ...
                  <tr key={i} style={{ borderBottom: "1px solid #0a1628" }}>
                    {result.headers.map((h, j) => (
                      <td key={h+j} style={{
                        ...
                        color: h === "Loyalty ID/Rewards Number" ? "#22c55e" : h === "Management Account Number / Retail Control Number" ? "#f59e0b" : "#94a3b8",
                        background: h === "Loyalty ID/Rewards Number" ? "#0a1e0f" : h === "Management Account Number / Retail Control Number" ? "#120e00" : "transparent",
                        fontWeight: h === "Loyalty ID/Rewards Number" || h === "Management Account Number / Retail Control Number" ? 700 : 400
                      }}>
