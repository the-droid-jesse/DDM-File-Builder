...
- **CSV & XLSX support** — Drag-and-drop or click to upload both file types
...
- **Handles large files** — Tested with 33,000+ row DDM files (~3 MB output)
...
| 5 | Loyalty ID/Rewards Number | **UUID looked up from reference file** |
...
4. **Run Mapping** — Click **▶ Run Mapping** to process all rows
...
| XLSX parsing | JSZip (loaded from CDN on demand) |
